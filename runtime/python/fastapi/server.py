...
async def lifespan(app: FastAPI):
    ...
    if model_dir:
        logging.info("MODEL_DIR is {}".format(model_dir))
        app.cosyvoice = CosyVoice(model_dir)
        # sft usage
        logging.info(
            "Avaliable speakers {}".format(app.cosyvoice.list_avaliable_spks())
        )
    else:
...
def buildResponse(output):
    filepath = "output.wav"
    torchaudio.save(filepath, output, 22050, format="wav")
    return FileResponse(
        path=filepath,
...
async def zeroShot(tts: str = Form(), prompt: str = Form(), audio: UploadFile = File()):
    ...
    prompt_speech_16k = torch.from_numpy(
        np.array(np.frombuffer(prompt_audio, dtype=np.int16))
    ).unsqueeze(dim=0)
