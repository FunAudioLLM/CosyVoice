...
def main():
    print('Loading CosyVoice3 with vLLM...', flush=True)
    t0 = time.time()
    cosyvoice = AutoModel(model_dir='pretrained_models/Fun-CosyVoice3-0.5B', load_trt=False, load_vllm=True, fp16=False)
    ...
    for run in range(3):
        ...
        for i, j in enumerate(cosyvoice.inference_zero_shot(text, prompt_text, prompt_wav, stream=False)):
            out = f'/home/zhiqiang/vllm_test_{run}_{i}.wav'
