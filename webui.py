    ...
    with gr.Blocks(theme=seafoam) as demo:
        ...
        with gr.Group():
            with gr.Row():
                ...
                with gr.Column(scale=3):
                    instruction_text = gr.Markdown(
                        value="### 操作步骤\n" + instruct_dict[inference_mode_list[0]]
                    )
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    with gr.Row():
                        seed_button = gr.Button(value="\U0001F3B2", scale=0.2)
                        seed = gr.Number(value=0, label="随机推理种子", precision=0)

        with gr.Tabs():
            with gr.TabItem("Prompt 音频"):
                with gr.Row():
                    with gr.Column():
                        prompt_wav_upload = gr.Audio(
                            ...
                            type='filepath', 
                            label='选择prompt音频文件',
                            elem_id="prompt-upload"
                        )
                        gr.Markdown("*注意：上传的音频采样率不低于16khz*")
                    with gr.Column():
                        prompt_wav_record = gr.Audio(
                            sources='microphone', 
                            type='filepath', 
                            label='录制prompt音频文件',
                            elem_id="prompt-record"
                        )
            ...
            with gr.TabItem("Instruct 控制"):
                instruct_text = gr.Textbox(
                    label="输入instruct文本", 
                    lines=2, 
                    placeholder="请输入instruct文本，用于控制合成语音的风格、情感等特性...", 
                    value=''
                )
        ...
        # Replace another gr.Box() with gr.Group()
        with gr.Group():
        ...
        gr.HTML("""
        <div class="footer">
            <p>育音 EduVoiceLab | 由CosyVoice模型强力驱动</p>
        </div>
        """)
